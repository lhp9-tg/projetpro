...
function cURL($url, $dir, $motRecherche) {
	$curl = curl_init($url);
	curl_setopt_array($curl, [
		CURLOPT_CAINFO => 'C:\Users\thoma\OneDrive\Code\Mes projets\Projet Pro\APIv2\cert\Amazon_Root_CA_1.crt',
	...
	if ($data === false) {
		...
		if (curl_getinfo($curl, CURLINFO_HTTP_CODE) == 200) {
			file_put_contents($dir . '/' . $motRecherche . '.json', $data);	
...
if(isset($_POST['mot']) && !empty($_POST['mot'])) {
	...
	if($match != '' && (time()-filemtime($match) < 60)) {
		...
		$json = cURL($url, $dir, $motRecherche);
		var_dump($json);
	}
